...
export class EntryDetailFormComponent
  extends ActionProcess
  implements OnInit, AdComponent
{
  @Input() value: any;
  ...
  protected builselect() {
    this.articleService
      .get("with=article_name,supplier")
      .pipe(
        map((state: any) => {
            ...
            this.form = this.rebuild_select_control_items(
              ...
              data.map(
                (value) =>
                  ({
                    name:
                      value?.article_name?.label.toUpperCase() +
                      " (" +
                      value?.type_name +
                      "- lab: " +
